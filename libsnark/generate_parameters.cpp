    ...
    char* params_path, char* input_path)
{
    ...
    for (size_t i = 0; i <= m; ++i) {
      write_bn128_g1<ppT>(params, keypair.pk.A_query[i]);
    }
    
    // printf("=============g1================\n");
    // keypair.pk.A_query[0].to_affine_coordinates();
    // keypair.pk.A_query[0].print_coordinates();
    // printf("=============g1================\n");
    for (size_t i = 0; i <= m; ++i) {
      write_bn128_g1<ppT>(params, keypair.pk.B_query[i].h);
    }
    
    //int flag = 0;
    
    for (size_t i = 0; i <= m; ++i) {
        write_bn128_g2<ppT>(params, keypair.pk.B_query[i].g);
    }
    // printf("=============g2================\n");
    // keypair.pk.B_query[flag].g.to_special();
    // keypair.pk.B_query[flag].g.print_coordinates();
    // printf("=============g2================\n");
    for (size_t i = 0; i < m-1; ++i) {
      write_bn128_g1<ppT>(params, keypair.pk.L_query[i]);
    }
    
    for (size_t i = 0; i < d; ++i) {
      write_bn128_g1<ppT>(params, keypair.pk.H_query[i]);
    }
    fclose(params);
    ...
    Fr<ppT>::one().as_bigint().print_hex();
    for (size_t i = 0; i < m; ++i) {
      write_fr<ppT>(input, full_variable_assignment[i]);
    }
    full_variable_assignment[0].as_bigint().print_hex();
    printf("data[0]:%lx\n", (full_variable_assignment[0].as_bigint().data[0])&0xffffffff);
    printf("data[0]:%lx\n", (full_variable_assignment[0].as_bigint().data[0]>>32)&0xffffffff);
    ...
    // params = fopen(params_path, "r");
    // d = read_size_t(params);
    // m = read_size_t(params);
    // // auto P_1 = read_bn128_g1<ppT>(params);
    // auto P_2 = read_bn128_g2<ppT>(params);
    // printf("============read data start===============\n");
    // printf("param size d=%d, m=%d\n", d, m);
    // // P_1.print_coordinates();
    // P_2.print_coordinates();
...
int main(int argc, const char * argv[])
{
  ...
  generate_paramaters<bn128_pp>(log2_d_4753, "MNT4753-parameters", "MNT4753-input");
